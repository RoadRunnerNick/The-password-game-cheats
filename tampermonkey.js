// ==UserScript==
// @name         Password Game Interface Cheat
...
(function () {
  ...
  class GameHelp {
    constructor() {
      this._gameOpts = Object.values(document.querySelector("#__nuxt"))[0][
        ...
        "deps"
      ][1]["subs"][0]["vm"]["$options"]["parent"]["$store"]["$router"][
        "history"
      ]["current"]["matched"][0]["instances"].default["_scope"]["effects"][0][
        "deps"
      ][0]["subs"]["2"]["deps"][14]["subs"][0]["vm"];
      this.utils = Object.values(document.querySelector("#__nuxt"))[0][
        "_scope"
      ]["effects"][3]["deps"][0]["subs"][1]["vm"]["_scope"]["effects"][0][
        "deps"
      ][1]["subs"][0]["vm"]["$options"]["parent"]["$store"]["$router"][
        "history"
      ]["current"]["matched"][0]["instances"].default;
    }
    get currentCaptcha() {
    ...
    get finalPasswordText() {
      return this._gameOpts.finalPasswordDoc.content.content[0].content
        .content[0].text;
    }
    getMoonPhases() {
      return "🌑 🌒 🌓 🌔 🌕 🌖 🌗 🌘 🌑";
    ...
    typeFinalPassword() {
      ...
      const innerBox = document.querySelector(
        ".password-box-inner .ProseMirror"
      );
      innerBox.innerHTML = "";
  ...
  const checker = setInterval(function () {
    try {
      const check = Object.values(document.querySelector("#__nuxt"))[0][
        "_scope"
      ]["effects"][3]["deps"][0]["subs"][1]["vm"]["_scope"]["effects"][0][
        "deps"
      ][1]["subs"][0]["vm"]["$options"]["parent"]["$store"]["$router"][
